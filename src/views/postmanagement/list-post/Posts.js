...
const Users = ({ match }) => {
  ...
  const changeEditPage = (data) => {
    store.dispatch({
      type: "CHANGE_STATE",
      payload: {
        modulState: "Edit Data",
        HeadModal: "Edit Form User",
        ShowHideAl: "d-none",
      },
    });
      ...
      .then((result) => {
        if (result) {
          ...
          for (let i = 0; i < result.length; i++) {
            if (result[i]) {
              cek = result[i].split(":");
              if (cek[0] === "draft") setcountDraft(cek[1]);
              else if (cek[0] === "publish") setcountPublished(cek[1]);
              else {
  ...
  async function pageChange(newPage) {
    setPage(newPage);
    await fetch(
      "http://localhost:9333/article/5/" +
        5 * (parseInt(newPage) - 1) +
        "/publish"
    )
      .then((res) => res.json())
      .then((result) => {
    ...
    <CRow>
      <CCol xl={12}>
        <CCard>
          ...
          <CCardBody>
            <CModal show={modalDelConf} onClose={toggleDelConf}>
              <CModalHeader>
                <CModalTitle>Confirmation</CModalTitle>
              </CModalHeader>
              <CModalBody>You are about to move this item to thrash</CModalBody>
              <CModalFooter>
                <CButton color="secondary" onClick={toggleDelConf}>
                  Cancel
                </CButton>
                <CButton color="danger" onClick={(e) => MoveToThrash(e)}>
                  Proceed
                </CButton>
              </CModalFooter>
            </CModal>
            <CModal show={modalDataView} onClose={toggleDataView}>
              <CModalHeader
                closeButton
                className="text-primary d-flex justify-content-end"
              >
                {FormDataView.username}
              </CModalHeader>
              <CModalBody>
                <div className="row g-5 d-flex justify-content-center">
                  <div className="col">
                    <CLabel className="form-label text-info" color="secondary">
                      Username
                    </CLabel>
              ...
              <CModalFooter>
                {" "}
                <CButton color="secondary" onClick={toggleDataView}>
                  Close
                </CButton>
              </CModalFooter>
            </CModal>
                ...
                <CTabPane data-tab="published">
                  <CDataTable
                    ...
                    scopedSlots={{
                      button_td: (item) => (
                        <td>
                          ...
                          <CButton
                            ...
                            type="submit"
                            className="w-10 mr-2 d-inline-block"
                            size="sm"
                            color="success"
                          >
                      ...
                      index: (item, index) => (
                        <td>
                          <CBadge color="info">{index + 1}</CBadge>
                        </td>
                      ),
                      // 'Content':
                      //   (item) => (
                      ...
                      //     </td>
                      //   ),
                      Status: (item) => <td>{infoBadge(item.Status)}</td>,
                      id: (item) => <td>{primaryBadge(item.Id)}</td>,
                    }}
                  />
                  <CPagination
                    activePage={page}
                ...
                >
                  <CFormGroup>
                    ...
                    <CInput
                      ...
                      onChange={onFieldChange("Title").bind(this)}
                      name="Title"
                      minLength="20"
                      maxLength="100"
                  ...
                  <CFormGroup>
                    ...
                    <CInput
                      ...
                      minLength="3"
                      maxLength="100"
                      onChange={onFieldChange("Category").bind(this)}
